...
- `route_exists` for Next.js App Router routes, FastAPI app/router decorators, Flask decorators, Django URL patterns, and literal Express route calls.
...
- It uses simple file conventions for Next.js, decorator patterns for FastAPI apps and routers, Flask decorators, literal Django `path("route/")` URL patterns, and literal `app.get("/path")` / `router.post("/path")` patterns for Express.
