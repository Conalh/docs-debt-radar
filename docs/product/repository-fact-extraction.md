...
- `route_exists` for Next.js App Router and Pages Router routes, FastAPI app/router decorators, Flask decorators, Django URL patterns, and literal Express route calls.
...
- It uses simple App Router and Pages Router file conventions for Next.js, decorator patterns for FastAPI apps and routers, Flask decorators, literal Django `path("route/")` URL patterns, and literal `app.get("/path")` / `router.post("/path")` patterns for Express.
