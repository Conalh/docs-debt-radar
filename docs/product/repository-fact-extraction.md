...
- `route_exists` for Next.js App Router routes, FastAPI decorators, Flask decorators, and literal Express route calls.
...
- It uses simple file conventions for Next.js, decorator patterns for FastAPI and Flask, and literal `app.get("/path")` / `router.post("/path")` patterns for Express.
